...
function Distributions.fit{D}(::Type{CategoricalCPD{D}}, data::DataFrame, target::NodeName)
    d = fit(D, data[target])
    CategoricalCPD(target, NodeName[], Int[], D[d])
end
function Distributions.fit{D}(::Type{CategoricalCPD{D}}, data::DataFrame, target::NodeName,  parents::Vector{NodeName})
...
function Distributions.fit{D}(::Type{CategoricalCPD{D}}, data::DataFrame, target::NodeName,  parents::Vector{NodeName}, prior::Dirichlet)
...
function Distributions.fit(::Type{DiscreteCPD}, data::DataFrame, target::NodeName, parents::Vector{NodeName})
...
function Distributions.fit(::Type{DiscreteCPD}, data::DataFrame, target::NodeName, prior::Dirichlet)
    probs = ConjugatePriors.posterior_mode(prior, Categorical, data[target])
    CategoricalCPD(target, NodeName[], Int[], [Categorical(probs)])
end
function Distributions.fit(::Type{DiscreteCPD}, data::DataFrame, target::NodeName, parents::Vector{NodeName}, prior::Dirichlet)

    # with parents

    if isempty(parents)
        return fit(DiscreteCPD, data, target, prior)
    end
    ...
    for (q, parent_instantiation) in enumerate(product(dims...))
        ...
        probs = ConjugatePriors.posterior_mode(prior, Categorical, arr)
